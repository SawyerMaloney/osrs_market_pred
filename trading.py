...
def naive_trading_strategy(model, test_data, sequence_length, criterion, item_ids, initial_balance=10000):
    ...
    inventory = 0
    predicted_future_price = 0.0
    with torch.no_grad():
        for i in range(len(test_data) - sequence_length - 1):
            inputs = test_data[i:i + sequence_length, :]
            current_price = test_data[i, item_ids.index("566")].squeeze()
            print(f"current actual: {current_price:.2f}. predicted: {predicted_future_price:.2f}")
